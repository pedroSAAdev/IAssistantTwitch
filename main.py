...
def detect_twitch_bot_command(sock):
    ...
    elif 'PRIVMSG' in resp and 'wizebot' not in resp and '!patoche' in resp:
        user = resp.split('PRIVMSG')[0].split(':')[1].split('!')[0]
        msg = resp.split('PRIVMSG')[1].split(':')
        res = msg[1].replace('!patoche','')
...
def generate_script_gpt(text,messages_prev):
    if len(messages_prev) == 0:
        messages_prev = [
            {"role": "system", "content": """Tu es l'assistant IA de <Streamer>,
            un streamer et youtuber sur Twitch. Tu dois répondre de manière drôle et atypique, à ce que <Streamer> te dis.
